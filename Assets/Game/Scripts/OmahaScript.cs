...
public class OmahaScript : MonoBehaviour
{
    ...
    private List<int> player4 = new List<int>(); //for player
    private List<int> mid5 = new List<int>(); //for opponent
    private List<int> oppo4 = new List<int>(); //for opponent
    ...
    private void ResetToCardBacks()
    {
        ...
        foreach (var item in midCardsGO)
        {
            item.GetComponent<CardScript>().GetCardBack();
    ...
    public void StartOmaha()
    {
        ...
        scoreOfPlayer = GetMaxScoreForPlayer();

        scoreOfOpponent = GetMaxScoreForOpponent();
    ...
    private void prepareCards()
    {
        ...
        card = mainCards.GetComponent<MainCards>().theFiveCard[3];
        player4.Add(CardIntValue(card));

        //adding opponent cards to evaluate list
        card = deck.DrawCard();
        opponentCards.Add(card);
        oppo4.Add(CardIntValue(card));
    ...
    private int GetMaxScoreForPlayer()
    {
        bestHandForPlayer.Clear();

        int score = 0;
        int maxScore = 0;
        List<int> evaluatorList = new List<int>();

        List<int> tmpListMid = new List<int>();
        List<int> tmpListPlayer = new List<int>();


        List<List<int>> midCombinations = GenerateCombinations(mid5, 3);
        List<List<int>> playerCombinations = GenerateCombinations(player4, 2);


        for (int i = 0; i < midCombinations.Count; i++)
        {
            tmpListMid = midCombinations[i];

            for (int j = 0; j < playerCombinations.Count; j++)
            {
                evaluatorList.Clear();

                tmpListPlayer = playerCombinations[j];


                foreach (var item in tmpListPlayer)
                {
                    evaluatorList.Add(item);
                }

                foreach (var item in tmpListMid)
                {
                    evaluatorList.Add(item);
                }

                evaluatorList = evaluatorList.OrderByDescending(x => x % 100).ToList();

                score = ScoreHand(evaluatorList.ToArray());
                ...
                foreach (var item in evaluatorList)
                {
                    ...
                    if (modScore > maxScore)
                    {
                        ...
                        for (int k = 0; k < 5; k++)
                        {
                            bestHandForPlayer.Add(evaluatorList[k]);
                        }
    ...
    private int GetMaxScoreForOpponent()
    {
        ...
        List<int> tmpListMid = new List<int>();
        List<int> tmpListOpponent = new List<int>();

        List<List<int>> midCombinations = GenerateCombinations(mid5, 3);
        List<List<int>> OpponentCombinations = GenerateCombinations(oppo4, 2);
        ...
        for (int i = 0; i < midCombinations.Count; i++)
        {
            ...
            for (int j = 0; j < OpponentCombinations.Count; j++)
            {
                evaluatorList.Clear();

                tmpListOpponent = OpponentCombinations[j];


                foreach (var item in tmpListOpponent)
                {
                    evaluatorList.Add(item);
                }
                ...
                foreach (var item in evaluatorList)
                {
                    ...
                    if (modScore > maxScore)
                    {
                        ...
                        for (int k = 0; k < 5; k++)
                        {
                            bestHandForOpponent.Add(evaluatorList[k]);
                        }
                    }
    ...
    {
        int tmpScore = PlayerPrefs.GetInt("BestOmahaScore");
        
        if(score > tmpScore)
        {
            PlayerPrefs.SetInt("BestOmahaScore",score);

            PlayerPrefs.SetInt("BestHandOmaha_0",h0);
            PlayerPrefs.SetInt("BestHandOmaha_1",h1);
            PlayerPrefs.SetInt("BestHandOmaha_2",h2);
            PlayerPrefs.SetInt("BestHandOmaha_3",h3);
            PlayerPrefs.SetInt("BestHandOmaha_4",h4);
