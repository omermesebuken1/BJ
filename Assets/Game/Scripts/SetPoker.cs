...
public class SetPoker : MonoBehaviour
{
        ...
        if (gameEnd)
        {
            if (scoreOfPlayer == scoreOfOpponent)
            {
                ...
                switch (winner)
                {
                    ...
                    UpdateBestHands(scoreOfPlayer, bestHandForPlayer[0], bestHandForPlayer[1], bestHandForPlayer[2]);
                    break;
    ...
    public void StartSetPoker()
    {
        startButton.SetActive(false);
        gm.Vibrate("soft");
        prepareCards();

        scoreOfPlayer = GetScore(evaluateList1, 1);

        scoreOfOpponent = GetScore(evaluateList2, 2);
    ...
    private int GetScore(List<int> tmpList, int whichPlayer)
    {
        int score = 0;
    ...
    public static int ScoreStraight(int[] hand)
    {
        ...
        if (hand[0] % 100 == 12 && hand[1] % 100 == 1 && hand[2] % 100 == 0)
        {
            return 4000;
        }
    ...
    private int Kickers()
    {
        ...
        if (scoreOfPlayer == scoreOfOpponent)
        {
            ...
            if (scoreOfPlayer == 4000 || scoreOfPlayer == 9000)
                ...
                && bestHandForPlayer[1] % 100 == 1
                && bestHandForPlayer[2] % 100 == 0)
                {
                    for (int i = 0; i < 5; i++)
                    {
                        if (bestHandForOpponent[i] % 100 > 1 && bestHandForOpponent[i] % 100 < 12)
                        {
                            return -1;
                        }
                    }
            ...
            && bestHandForOpponent[1] % 100 == 1
            && bestHandForOpponent[2] % 100 == 0)
            {
                for (int i = 0; i < 5; i++)
                {
                    if (bestHandForPlayer[i] % 100 > 1 && bestHandForPlayer[i] % 100 < 12)
                    {
                        return 1;
                    }
                }
    ...
    private void UpdateBestHands(int score, int h0, int h1, int h2)
    {
        int tmpScore = PlayerPrefs.GetInt("BestSetPokerScore");
        
        if(score > tmpScore)
        {
            PlayerPrefs.SetInt("BestSetPokerScore",score);

            PlayerPrefs.SetInt("BestHandSetPoker_0",h0);
            PlayerPrefs.SetInt("BestHandSetPoker_1",h1);
            PlayerPrefs.SetInt("BestHandSetPoker_2",h2);
