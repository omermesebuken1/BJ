...
public class CrazyPoker : MonoBehaviour
{
        ...
        if (gameEnd)
        {
            if (scoreOfPlayer == scoreOfOpponent)
            {
                ...
                switch (winner)
                {
                    ...
                    UpdateBestHands(scoreOfPlayer, bestHandForPlayer[0], bestHandForPlayer[1], bestHandForPlayer[2], bestHandForPlayer[3]);
                    break;
    ...
    public void StartCrazyPoker()
    {
        ...
        scoreOfPlayer = GetMaxScore(GenerateCombinations(evaluateList1, 4), 1);

        scoreOfOpponent = GetMaxScore(GenerateCombinations(evaluateList2, 4), 2);
    ...
    private void prepareCards()
    {
        ...
        card = mainCards.GetComponent<MainCards>().theFiveCard[4];
        evaluateList1.Add(CardIntValue(card));
    ...
    public static int ScoreStraight(int[] hand)
    {
        ...
        if (hand[0] % 100 == 12 && hand[1] % 100 == 2 && hand[2] % 100 == 1 && hand[3] % 100 == 0)
        {
            return 4000;
        }
    ...
    private int Kickers()
    {
        ...
        if (scoreOfPlayer == scoreOfOpponent)
        {
            ...
            && bestHandForPlayer[1] % 100 == 2
            && bestHandForPlayer[2] % 100 == 1
            && bestHandForPlayer[3] % 100 == 0)
            {
                for (int i = 0; i < 5; i++)
                {
                    if(bestHandForOpponent[i] % 100 > 2 && bestHandForOpponent[i] % 100 < 12)
                    {
                        return -1;
                    }
                }
            ...
            && bestHandForOpponent[1] % 100 == 2
            && bestHandForOpponent[2] % 100 == 1
            && bestHandForOpponent[3] % 100 == 0)
            {
                for (int i = 0; i < 5; i++)
                {
                    if(bestHandForPlayer[i] % 100 > 2 && bestHandForPlayer[i] % 100 < 12)
                    {
                        return 1;
                    }
                }
    ...
    private void ResetWinnerHands()
    {
       ...
       private void UpdateBestHands(int score, int h0, int h1, int h2, int h3)
    {
        int tmpScore = PlayerPrefs.GetInt("BestCrazyPokerScore");
        
        if(score > tmpScore)
        {
            PlayerPrefs.SetInt("BestCrazyPokerScore",score);

            PlayerPrefs.SetInt("BestHandCrazyPoker_0",h0);
            PlayerPrefs.SetInt("BestHandCrazyPoker_1",h1);
            PlayerPrefs.SetInt("BestHandCrazyPoker_2",h2);
            PlayerPrefs.SetInt("BestHandCrazyPoker_3",h3);
